...
export function getDb() {
  ...
  const blockColumns = _db.prepare("PRAGMA table_info(blocks)").all();
  if (!blockColumns.some(col => col.name === 'cards_per_row')) {
    _db.exec("ALTER TABLE blocks ADD COLUMN cards_per_row INTEGER NOT NULL DEFAULT 2");
  }

  const countBlocks = _db.prepare('SELECT COUNT(*) as count FROM blocks').get();

  if (countBlocks.count === 0) {
    console.log('Seeding initial database data...');

    const insertTransaction = _db.transaction(() => {
      const insertBlock = _db.prepare('INSERT INTO blocks (name, sort_order) VALUES (?, ?)');
      const insertLink = _db.prepare('INSERT INTO links (block_id, title, url, sort_order) VALUES (?, ?, ?, ?)');
